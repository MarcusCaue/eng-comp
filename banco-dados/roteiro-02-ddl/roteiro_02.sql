...
ALTER TABLE Venda
ADD
    CONSTRAINT Fk_venda_cliente FOREIGN KEY (codigo_cli_fk) REFERENCES Cliente (codigo_cli_pk);
...
CREATE TABLE
    Item_venda (
        ...
        CONSTRAINT Pk_item_venda PRIMARY KEY (
            codigo_prod_fk,
            numero_venda_fk
        )
    );
...
ADD
    CONSTRAINT Fk_item_venda_prod FOREIGN KEY (codigo_prod_fk) REFERENCES Produto (codigo_pk);

ALTER TABLE Item_venda
ADD
    CONSTRAINT Fk_item_venda_venda FOREIGN KEY (numero_venda_fk) REFERENCES Venda (numero_id);
